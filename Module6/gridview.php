...
if ($data && is_array($data)){
    ...
    if (!empty($searchQuery)) {
        $data = array_filter($data, function($row) use ($searchQuery) {
            // Check multiple fields for a match
            return (strpos(strtolower($row['showid']), $searchQuery) !== false) ||
                   (strpos(strtolower($row['name']), $searchQuery) !== false) ||
                   (strpos(strtolower($row['genre']), $searchQuery) !== false) ||
                   (strpos(strtolower($row['country']), $searchQuery) !== false) ||
                   (strpos(strtolower($row['service']), $searchQuery) !== false);
        });
    }
    ...
    echo "  <input type='text' name='search' id='searchInput' value='" . htmlspecialchars($searchQuery) . "' placeholder='Search for shows, genres, services...'>";
    ...
    // Previous link
    if ($currentPage > 1) {
       echo '<a href="?page=' . ($currentPage - 1) . '&sort=' . $sortColumn . '&order=' . $sortOrder . '&search=' . urlencode($searchQuery) . '">Previous</a> ';
    }
    ...
    for ($i = 1; $i <= $totalPages; $i++) {
        if ($i == $currentPage) {
            ...
            echo '<a href="?page=' . $i . '&sort=' . $sortColumn . '&order=' . $sortOrder . '&search=' . urlencode($searchQuery) . '">' . $i . '</a> ';
        }
    }

    // Next link
    if ($currentPage < $totalPages) {
        echo '<a href="?page=' . ($currentPage + 1) . '&sort=' . $sortColumn . '&order=' . $sortOrder . '&search=' . urlencode($searchQuery) . '">Next</a>';
    }
